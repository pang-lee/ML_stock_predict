...
def compute_technical_indicators(data, window=14, plot_vpfr=False):
    if len(data) < window:
    ...
    # macd計算
    short_ema = data['close'].ewm(span=3, adjust=False).mean()  # 短期EMA
    long_ema = data['close'].ewm(span=7, adjust=False).mean()  # 長期EMA
    ...
    # 將MACD和Signal作為返回的結果
    macd_value = macd.iloc[-1]  # 最新的MACD值
    signal_value = signal.iloc[-1]  # 最新的Signal值
    macd_data = {"macd_value": macd_value, "signal_value": signal_value}
    
    # 計算VPFR
    price_bins = pd.interval_range(
        start=data['lowest_low'].iloc[-1], 
        end=data['highest_high'].iloc[-1], 
        freq=(data['highest_high'].iloc[-1] - data['lowest_low'].iloc[-1]) / window
    )
...
def check_vpfr(vpfr, data, params, is_trend=False, debug=False):
    ...
    # 根據 VPFR 判斷盤勢類型
    if is_trend:
        if high_price_vpfr > params.get('vpfr_trend') or low_price_vpfr > params.get('vpfr_trend'):
            if debug:
                print("✅ 趨勢盤: 成交量集中於極端價位")
            return True
        else:
            if debug:
                print("❌ 不符合趨勢盤條件")
...
def trade_trend(data1, data2, indicator, params, debug=False):
    """
    判斷趨勢盤條件並執行交易。
    
    :param data1: 價格, data2: 斜率1, data3: 斜率2
    :param indicator: 技術指標字典 (包括rsv和vpfr)
    :param volume_threshold: 成交量閾值
    """
    ...
    # 判斷趨勢盤條件
    if check_vpfr(vpfr, data2, is_trend=True, params=params, debug=debug) is True:
        macd = indicator['macd1']

        print("趨勢盤確認，執行交易")
        
        # 判斷市場方向
        if  macd['macd_value'] > 0 and  macd['signal_value'] > 0:
            print("多頭趨勢，執行多單交易")
            return (True, 1)
        elif  macd['macd_value'] < 0 and  macd['signal_value'] < 0:
            print("空頭趨勢，執行空單交易")
            return (True, -1)
        else:
            return (False, 0)
...
def bilateral_strategy( **params):
    ...
    indicators_data = {'time': [], 'rsv': [], 'vpfr': [], 'macd': []}  # 用來存儲所有的技術指標數據
    ...
    for i in range(1, len(t_d)):
        ...
        if current_minute != last_checked_minute:
            ...
            indicator1 = compute_technical_indicators(recent_price1, window=long_window, plot_vpfr=False)
            indicator2 = compute_technical_indicators(recent_price2, window=long_window, plot_vpfr=False)
            ...
            indicators_data['vpfr'].append(indicator2['vpfr'])
            indicators_data['macd'].append(indicator2['macd'])
            ...
            combined_indicators = {
                ...
                'macd1': indicator1['macd'],
                'macd2': indicator2['macd']
            }
            ...
            if is_stationary_long:
                ...
                trade_signal = trade_trend(recent_price1, recent_price2, combined_indicators, params=params, debug=False)
...
params = {
    ...
    'vpfr_trend': 0.2,
    'vpfr_oscillation': 0.45,
    'rsv_low': 20,
    'rsv_high': 80,
    'oscillation_buffer': 0.05, # 震盪緩衝(支撐壓力的價差區間)
    'volume_ratio': 0.6,
    'volume_threshold': 600, # 指數成交量門檻
    'volume_slippage': 100, # 時間段內成交量小於指定數值不交易(震盪盤滑價)
    'bid_ask_slippage': 0.1, # 委買賣滑價價差(tick)
    ...
    'capital': 100000,
    'stop_ratio1': 0.005, # 震盪盤止損
    'dynamic_stop_ratio1': 0.005, # 趨勢盤止損
    'dynamic_profit_ratio1': 0.01, # 趨勢盤止盈
